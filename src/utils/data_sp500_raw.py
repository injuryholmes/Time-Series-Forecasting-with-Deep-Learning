...
class SP500(Dataset):
    def __init__(self, folder_dataset, T=20, symbol='AAPL', use_columns=['date', 'open', 'high','low','close'], target='close', start_date='2012-01-01', end_date='2015-12-31', step=1):
        ...
        tmp = ['open', 'high', 'low','close','Target','close_t-1']
    ...
    def __getitem__(self, index):
        close_t_1 = self.chunks[index, -1 , -1] # yesterday's close price
        x = self.chunks[index, :-1, :-2] # previous features
        y = self.chunks[index, -1, -2] # tomorrow's price
