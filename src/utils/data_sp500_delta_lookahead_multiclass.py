...
class SP500(Dataset):
    def __init__(self, folder_dataset, T=20, symbol='AAPL', use_columns=['date', 'open', 'high','low','close'], target='close', start_date='2012-01-01', end_date='2015-12-31', step=1):
        ...
        tmp = ['Close_t-1','Close_t-Close_t-1', 'Close_t-Open_t', 'Open_t-Close_t-1', 'Open_t+1-Close_t','Target','multiclass']
        # breakpoint()
        ...
        self.df_data['change_ratio'] = self.df_data['Close_t-Close_t-1'].abs() / self.df_data['Close_t-1']
        self.df_data['multiclass'] = 0
        # breakpoint()
        for i, row in self.df_data.iterrows():
            # if row['change_ratio'] >= 0.09 and row['Close_t-Close_t-1']<0:
            #     self.df_data.at[i,'multiclass'] = 1
            # elif row['change_ratio'] >= 0.09 and row['Close_t-Close_t-1']>0:
            #     self.df_data.at[i,'multiclass'] = 2
            # elif row['change_ratio'] > 0.01 and row['Close_t-Close_t-1']<0:
            #     self.df_data.at[i,'multiclass'] = 3
            # elif row['change_ratio'] > 0.01 and row['Close_t-Close_t-1']>0:
            #     self.df_data.at[i,'multiclass'] = 4
            if row['change_ratio'] >= 0.015 and row['Close_t-Close_t-1']<0:
                self.df_data.at[i,'multiclass'] = 1
            elif row['change_ratio'] >= 0.015 and row['Close_t-Close_t-1']>0:
                self.df_data.at[i,'multiclass'] = 2
    ...
    def __getitem__(self, index):
        ...
        x = self.chunks[index, :-1, -6:-2] # delta features
        y = self.chunks[index, -1, -2] #  price to predict 
        multiclass = self.chunks[index, -1, -1]
        return close_t_1, x, y, multiclass
