...
# 16*7*19 -conv1-> 16*64*17 -conv2-> 16*64*15 -conv3-> 16*64*13 
# -conv4-> 16*64*11 -conv_final-> 16*7*10 -out[:,:,-1]->16*7 
...
class DilatedNet(nn.Module):
    def __init__(self, num_securities=5, hidden_size=64, dilation=2, T=10):
        ...
        # Layer 2
        self.dilated_conv2 = nn.Conv1d(hidden_size, hidden_size, kernel_size=2, dilation=self.dilation)
        self.relu2 = nn.ReLU()

        # Layer 3
        self.dilated_conv3 = nn.Conv1d(hidden_size, hidden_size, kernel_size=2, dilation=self.dilation)
        self.relu3 = nn.ReLU()

        # Layer 4
        self.dilated_conv4 = nn.Conv1d(hidden_size, hidden_size, kernel_size=2, dilation=self.dilation)
        self.relu4 = nn.ReLU()

        # Output layer
        self.conv_final = nn.Conv1d(hidden_size, num_securities, kernel_size=2)
...
# Original version of DCNN2D
# 16*1*7*19 -conv1-> 16*64*7*17 -conv2->  16*64*7*15 -conv3-> 16*64*7*13 
#  -conv4-> 16*64*7*11 -conv_final-> 16*1*7*10 -> 16*1*7
class DilatedNet2D(nn.Module):
    def __init__(self, hidden_size=64, dilation=2, T=20):
        """
        :param hidden_size: int, size of hidden layers
    ...
    def forward(self, x):
        ...
        # Final layer
        out = self.conv_final(out)
        out = out[:, :, :, -1] # why is this step only using the final col of the final dim
