...
file = pd.read_csv('data/stock/sandp500/individual_stocks_5yr/'+cfg.symbol+'_data.csv', index_col='date', parse_dates=True)
...
test_end =  pd.to_datetime("2017-06-01")
delta_days = test_end - train_start
train_end = train_start +delta_days*0.6
test_start = test_end - delta_days*0.2
######

###### Shanghai index -- version 1
###### This block specifies the start and end dates of Shanghai index used in one published paper
# train_start = pd.to_datetime("2004-01-01")
# test_end =  pd.to_datetime("2019-06-01")
# delta_days = test_end - train_start
# train_end = train_start +delta_days*0.6
# test_start = test_end - delta_days*0.2    
######
...
if __name__ == "__main__":
    ...
    plt.plot(np.array(x), test_mses, label="test mse",color='red')
    ...
    # # plt.plot(x, loss_time[:], label="loss_time", color=cm.Blues(300))
    # # plt.show()
